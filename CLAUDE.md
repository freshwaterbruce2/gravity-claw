...
## What this is
AI agent orchestrator desktop app — Tauri 2 wrapper around a Vite + React UI + Hono API server, featuring the "gravity mechanic" for multi-platform messaging (Telegram, Discord, email) powered by kimi-k2.5.

## Stack
- **Runtime**: Node.js 22 + Tauri 2 (Rust)
- **Framework**: Vite + React 19 (UI) + Hono (API) + Tauri (desktop shell)
...
pnpm --filter gravity-claw desktop:dev  # Full Tauri desktop mode
...
- Tauri build target: Windows NSIS/MSI installer (`src-tauri/target/release/bundle/`)
